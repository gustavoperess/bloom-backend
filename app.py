...
def create_user():
    ...
    try:
        ...
        email_exists = user_repository.email_exists(email)
        if user_exists[0]['exists']:
            return  jsonify({"message": "Bad request - user not created. This username has already been taken."}),401
        elif email_exists[0]['exists']:
            return  jsonify({"message": "Bad request - user not created. This email has already been taken."}),401
        else:
...
def get_help_requests_with_plant_photo_and_user_details():
    connection = get_flask_database_connection(app)
    request_repository = HelpRequestRepository(connection)
    help_requests_with_details = request_repository.get_plant_photo_with_user_details_for_help_request()
